...
function computeInput(depth, transcript, nullifier) {
    ...
    transcript.forEach((item )=> {
      if(item.length > 1){
        merkleTree.insert(mimc2(item[0],item[1]));
        if(item[0] == nullifier){
          nonce = item[1]
        }
      }else {
    ...
    let digest = merkleTree.digest;
    let res ={digest:digest,nonce:nonce,nullifier:nullifier};
    if(depth > 0){
      let path = merkleTree.path(mimc2(nullifier,nonce));
      path.forEach((item,idx)=> {
        res["direction["+idx+"]"] = String(item[1]? 1 : 0);
        res["sibling["+idx+"]"] = String(item[0]);
      })    
    }
